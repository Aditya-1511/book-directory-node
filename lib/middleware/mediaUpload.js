...
function parseExcelFile() {
	return function (request, response, next) {
		var files = _fetchFilesFromReq(request);
		var file = files[0];
		var wb = xlsx.readFile(file.path, { cellDates: true });
		var data;
		var ws;
		// console.log(wb, "my file");
		// console.log(files, "%%%%%%%%%%%%%%123")
		let allData = []

		wb.SheetNames.map((res, index) => {
			ws = wb.Sheets[res];
			...
			data.map(agItems => {
				allData.push(agItems)
			}) //to push all the ag items from every sheet into single array.

		})

		if (data.length) {
			request.body.uploadFile = allData
		}
		next()
	}
}
